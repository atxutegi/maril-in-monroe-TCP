...
public class TCPbasedTest extends AbstractTCPbasedTest implements Callable<ThreadTestResult>       
{
...
//                resultNsec = Arrays.copyOf(resultNsec, results);
    ...
    public TCPbasedTest(final marilClientParam client, final testParameters params, final int threadId,
    ...
    protected Socket connect(final TestResult testResult) throws IOException
    {
        ...
        final Socket s = getSocket(inetAddress.getHostAddress(), params.getPort(), 20000);
        ...
        final String send = String.format(Locale.US, "PASSPHRASE %s\n", params.getToken());
        ...
        if (line == null)
        {
            log(String.format(Locale.US, "thread %d: got no answer expected 'ACK'", threadId, line));
            return null;
        }
        else if (!line.equals("ACK"))
        {
            log(String.format(Locale.US, "thread %d: got '%s' expected 'ACK'", threadId, line));
            return null;
        }
        ...
        try
        {
            if (!"FRAMESIZE".equals(scanner.next()))
            {
    ...
    private void downloadFrames(final int frames) throws InterruptedException, IOException
    {
        ...
        if (!line.equals("WAITING FOR COMMAND"))
        {
            log(String.format(Locale.US, "thread %d: got '%s' expected 'WAITING FOR COMMAND'", threadId, line));
            ...
            throws IOException, UnsupportedEncodingException, InterruptedException, IllegalStateException
    {
        ...
        String send;
        send = String.format(Locale.US, "DOWNLOAD %d\n", seconds);
        out.write(send.getBytes("US-ASCII"));
        out.flush();
            ...
            UnsupportedEncodingException, InterruptedException, IllegalStateException
    {
        ...
        final Future<Boolean> futureRx = marilClientParam.getCommonThreadPool().submit(new Callable<Boolean>()
        {
        ...
        final Future<Void> futureTx = marilClientParam.getCommonThreadPool().submit(new Callable<Void>()
        {
